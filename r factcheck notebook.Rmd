...
agedist_test = read_excel('tabula-Age_BOP_FY2015-21_cleaned.xlsx')
...
## checks that I need to talk about jingnan too

death_year_age$deaths %>% sum()

nrow(deathmaster_bop)

length(unique(deathmaster_bop$RegisterNum))

deathmaster_bop %>% group_by(RegisterNum) %>%  count() %>% View()
```
