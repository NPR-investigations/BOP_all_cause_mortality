...
# cleaning up the columns as well as dropping duplicates - some of the requests have overlapping time frames.
...
```
```{r}
# calculate risk ratio confidence intervals for each age group
```


```{r}
confidence_interval <- function(df,event_treatment, no_event_treatment, event_control, no_event_control){
  df_result = df %>% 
  mutate(rr = (df[[event_treatment]]/(df[[event_treatment]]+df[[event_control]])) / (df[[no_event_treatment]]/(df[[no_event_treatment]] + df[[no_event_control]]))) %>% 
  mutate(total_exposed = df[[event_treatment]]+df[[event_control]],
         total_control = df[[no_event_treatment]] + df[[no_event_control]],
         block1 = ((total_exposed - df[[event_treatment]])/df[[event_treatment]])/total_exposed,
         block2 =  ((total_control - df[[no_event_treatment]])/df[[no_event_treatment]]) / total_control,
         sqrt_blocks = sqrt(block1 + block2),
         ci_lower = exp(log(rr) - (1.96*sqrt_blocks)),
         ci_upper = exp(log(rr) + (1.96*sqrt_blocks))) %>% 
    select(-total_exposed, total_control, -block1, -block2, -sqrt_blocks)
 return(df_result )
...
ref_ci = std %>% 
  select(age_group, group_pop, deaths) %>% 
  mutate(alive = group_pop - deaths) %>% 
  select(-group_pop) %>% 
  rename(ref_deaths = deaths, ref_alive = alive)

yr20_ci = yr20_3 %>% 
  select(age_group, group_pop.x, deaths.x) %>% 
  rename(group_pop_20 = group_pop.x, deaths_20 = deaths.x) %>% 
  mutate(alive_20 = group_pop_20 - deaths_20) %>% 
  select(-group_pop_20)


ci_20 = full_join(ref_ci, yr20_ci) %>% 
  confidence_interval( "deaths_20", "ref_deaths", "alive_20", "ref_alive") 
...
yr21_ci = yr21_3 %>% 
  select(age_group, group_pop.x, deaths.x) %>% 
  rename(group_pop_21 = group_pop.x, deaths_21 = deaths.x) %>% 
  mutate(alive_21 = group_pop_21 - deaths_21) %>% 
  select(-group_pop_21)


ci_21 = full_join(ref_ci, yr21_ci) %>% 
  confidence_interval("deaths_21", "ref_deaths", "alive_21", "ref_alive")
